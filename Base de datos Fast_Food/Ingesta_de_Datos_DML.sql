...
--La solucion es desactivar las FK momentaneamente y luego volver a activarlas:
--Desactivar FK Tabla DetalleOrdenes, Columna ProductoID
ALTER TABLE	DetalleOrdenes
NOCHECK CONSTRAINT FK__DetalleOr__Produ__6383C8BA;
--Eliminar la Categoria 2 (Postres)
--Activar FK Tabla DetalleOrdenes, Columna ProductoID
ALTER TABLE	DetalleOrdenes
...
--Registros únicos: Pregunta 1: ¿Cuál es la cantidad total de registros únicos en la tabla de órdenes?
SELECT  COUNT(OrdenID) AS CantidadOrdenes
FROM Ordenes;

--Empleados por departamento: Pregunta 2: ¿Cuántos empleados existen en cada departamento?
SELECT Departamento, COUNT(EmpleadoID) AS Cantidad
FROM Empleados
GROUP BY Departamento
ORDER BY Cantidad DESC;

--Productos por categoría: Pregunta 3: ¿Cuántos productos hay por código de categoría?
SELECT CategoriaID, COUNT(ProductoID) AS Cantidad
FROM Productos
GROUP BY CategoriaID

--Importación de clientes: Pregunta 4: ¿Cuántos clientes se han importado a la tabla de clientes?
SELECT COUNT(ClienteID) AS TotalClientes
FROM Clientes;

--Análisis de desempeño de sucursales
/* Pregunta 5: ¿Cuáles son las sucursales con un promedio de ventas por orden superior a un valor específico, 
ordenadas por el promedio de kilómetros recorridos para las entregas de mayor a menor?*/
SELECT SucursalID,
	FORMAT(AVG(TotalCompra), '0.00') AS PromedioVenta,
	FORMAT(AVG(KilometrosRecorrer), '0.00') AS PromedioKilometros
FROM Ordenes
GROUP BY SucursalID
